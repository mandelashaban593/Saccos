...
$sql="SELECT * FROM contacts_setup";
$result=mysqli_query($db,$sql);
$rowcount=mysqli_num_rows($result);
if($rowcount==0){
	$sql="INSERT INTO contacts_setup (`shortdesc`, `heading`, `subheading`, `dob`, `website`, `longdesc`)
VALUES ('$shortdesc','$subheading', '$heading','$dob', '$website', '$longdesc')";

$query=mysqli_query($db,$sql) or die(mysql_error($db));
if($query){
header("location:../?editcontacts=true&msg=updated");
}

} else {

$query="UPDATE contacts_setup SET ";
