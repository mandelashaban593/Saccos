...
$queryrun=mysqli_query($db,$query);
if($queryrun){
    header("location:../?editsavings=true#done");
}    
...
if($pdone=="error"){
    header("location:../?editsavings=true&msg=error");
}else{
$query="INSERT INTO savings (requiredescription,projectpic,projectlink,shortheading,heading,subdescribe) ";
$query.="VALUES ('$requiredescription','$projectpic','$projectlink','$shortheading','$heading','$subdescribe')";
$queryrun=mysqli_query($db,$query);
if($queryrun){
    header("location:../?editsavings=true&msg=updated");
}    
