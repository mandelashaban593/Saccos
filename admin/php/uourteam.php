...
$queryrun=mysqli_query($db,$query);
if($queryrun){
    header("location:../?editourteam=true#done");
}    
...
if(isset($_GET['del'])){
    $id=$_GET['del'];
    $query="DELETE FROM ourteam WHERE id='$id'";
    $queryrun=mysqli_query($db,$query);
if($queryrun){
    header("location:../?editourteam=true#done");
}
...
if($pdone=="error"){
    header("location:../?editourteam=true&msg=error");
}else{
$query="INSERT INTO ourteam (projectname,projectpic,projectlink) ";
$query.="VALUES ('$projectname','$projectpic','$projectlink')";
$queryrun=mysqli_query($db,$query);
if($queryrun){
    header("location:../?editourteam=true&msg=updated");
}    
